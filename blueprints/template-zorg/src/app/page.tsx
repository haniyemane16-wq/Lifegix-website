...
function Behandelingen() {
  const { behandelingen, kleur } = clientConfig;
  return (
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-white/50 max-w-md mx-auto">
            Persoonlijke behandeling, gericht op herstel en preventie.
          ...
          {behandelingen.map((b) => (
            <div
              ...
              className={`relative rounded-2xl p-6 gradient-border flex flex-col gap-4 ${
                b.populair ? "bg-white/[0.06]" : "bg-white/[0.03]"
              }`}
            >
              {b.populair && (
                <span
                  ...
                  style={{
                    backgroundColor: `${kleur.primary}22`,
                    borderColor: `${kleur.primary}44`,
                    color: kleur.primary,
                  }}
                >
                  Meest gevraagd
              ...
              <div>
                <h3 className="text-base font-bold text-white pr-16">
                  {b.naam}
                </h3>
              ...
              <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
                <span className="text-xs text-white/30">{b.duur}</span>
                {b.vergoeding && (
                  <span
...
/* ─── Team ─────────────────────────────────────────── */
function Team() {
  const { team, kleur } = clientConfig;
  return (
    <section id="team" className="py-24 pb-12 px-6 relative">
      <div className="absolute inset-0 pointer-events-none hidden sm:block">
        <div
      ...
      <div className="max-w-6xl mx-auto relative">
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-white/50 max-w-md mx-auto">
            Gecertificeerd, ervaren en persoonlijk betrokken bij elk hersteltraject.
          </p>
        </div>

        <div className={`grid gap-6 max-w-3xl mx-auto ${team.length === 1 ? "" : "sm:grid-cols-2"}`}>
          {team.map((lid) => (
            <div
              key={lid.naam}
              className="p-6 rounded-2xl bg-white/[0.03] border border-white/10 flex flex-col gap-4"
            >
              {/* Foto placeholder of echte foto */}
              <div
                className="w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold text-white"
                style={{ backgroundColor: `${kleur.primary}33` }}
              >
                {lid.naam.split(" ").map((n) => n[0]).slice(0, 2).join("")}
              </div>
              <div>
                <h3 className="text-white font-bold">{lid.naam}</h3>
                <p
              ...
              <div className="space-y-1">
                <p className="text-white/50 text-sm">
                  <span className="text-white/30">Specialisatie:</span>{" "}
                  {lid.specialisatie}
                </p>
                <p className="text-white/50 text-sm">
                  <span className="text-white/30">Ervaring:</span>{" "}
                  {lid.ervaring}
                </p>
              </div>
            </div>
    ...
    <section id="werkwijze" className="py-24 pb-12 px-6 relative">
      ...
      <div className="max-w-6xl mx-auto relative">
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-white/50 max-w-md mx-auto">
            Duidelijkheid van dag één. Geen verrassingen, wel resultaat.
          </p>
        </div>
          ...
          {reviews.map((r) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-white/40 text-xs">{r.klacht}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start max-w-5xl mx-auto">
          <div>
            ...
            <div className="space-y-4">
              ...
              {/* Zorgverzekering info box */}
              <div
                className="mt-4 p-4 rounded-xl"
                style={{
                  backgroundColor: `${kleur.primary}11`,
                  border: `1px solid ${kleur.primary}33`,
                }}
              >
                <p
                ...
                <p className="text-white/50 text-xs leading-relaxed">
                  Fysiotherapie wordt vergoed vanuit aanvullende zorgverzekering.
                  Neem contact op voor een overzicht van uw vergoedingsrechten.
                </p>
              </div>
            </div>
          </div>
...
/* ─── Footer ───────────────────────────────────────── */
function Footer() {
  const { naam, stad, kvk, bigRegistratie } = clientConfig;
  return (
